...
// Package rlp implements the simple byte encoding/decoding using
...
func EncodeBytes(input []byte) ([]byte, error) {
	...
	length, err := encodeLength(len(input), 0x80)
	if err != nil {
		return nil, err
	}

	return append(length, input...), nil
}

// decodeLength returns the length of the RLP encoding prefix (offset) and the length of the encoded byte slice.
func decodeLength(input []byte) (offset int, length int, err error) {
	if len(input) == 0 {
	...
	if prefix < 0xc0 {
		...
		copy(lengthAsBytes[8-length:], input[1:length+1])

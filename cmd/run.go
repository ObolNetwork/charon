...
func bindRunFlags(cmd *cobra.Command, config *app.Config) {
	...
	cmd.Flags().StringVar(&config.ProcDirectory, "proc-directory", "", "Directory to look into in order to detect other stack components running on the host. Enabling this exports the command lines of detected validator clients to the monitoring endpoint and debug logs, with the values of secret-shaped flags redacted.")
	...
	wrapPreRunE(cmd, func(cc *cobra.Command, _ []string) error {
		...
		if config.FetchFeerecipientUpdates && config.PublishAddress == "" {
			return errors.New("--fetch-feerecipient-updates requires --publish-address to be set")
		}
...
func bindP2PFlags(cmd *cobra.Command, config *p2p.Config, defaultRelays ...string) {
	if len(defaultRelays) == 0 {
		cmd.Flags().StringSliceVar(&config.Relays, "p2p-relays", []string{"https://0.relay.obol.tech", "https://1.relay.obol.tech", "https://2.relay.obol.dev"}, "Comma-separated list of libp2p relay URLs or multiaddrs.")
	} else {
	...
	cmd.Flags().StringSliceVar(&config.UDPAddrs, "p2p-udp-address", nil, "Comma-separated list of listening UDP addresses (ip and port) for libP2P traffic. Empty default doesn't bind to local port therefore only supports outgoing connections.")

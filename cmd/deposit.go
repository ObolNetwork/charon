...
func newDepositCmd(cmds ...*cobra.Command) *cobra.Command {
	root := &cobra.Command{
		...
		Long:  "Sign and fetch a new partial deposit messages using a remote API.",
	}
...
func bindDepositFlags(cmd *cobra.Command, config *depositConfig) {
	cmd.Flags().StringSliceVar(&config.ValidatorPublicKeys, "validator-public-keys", []string{}, "List of validator public keys for which deposits will be signed.")

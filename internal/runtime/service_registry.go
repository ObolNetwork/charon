...
func (s *ServiceRegistry) StartAll() {
	log.Debug().Msgf("Starting %d services: %v", len(s.serviceTypes), s.serviceTypes)
	for _, kind := range s.serviceTypes {
		log.Debug().Msgf("Starting service type %v", kind)
...
func (s *ServiceRegistry) StopAll() {
	log.Debug().Msgf("Stopping %d services: %v", len(s.serviceTypes), s.serviceTypes)
	for i := len(s.serviceTypes) - 1; i >= 0; i-- {
		kind := s.serviceTypes[i]
		service := s.services[kind]
		if err := service.Stop(); err != nil {
			log.Err(err).Msgf("Could not stop the following service: %v", kind)
		}
	}
}

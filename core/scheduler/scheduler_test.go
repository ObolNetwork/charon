...
func TestScheduler_GetDutyDifferentPubkeys(t *testing.T) {
	var (
		ctx    = context.Background()
		t0     time.Time
		slot   = int64(1)
	...
	eth2Cl, err := beaconmock.New(
		...
		beaconmock.WithDeterministicSyncCommDuties(2, 2),
		beaconmock.WithSlotsPerEpoch(1),
		beaconmock.WithDutiesPubkeys(testutil.RandomEth2PubKey(t)),
	)
	require.NoError(t, err)
	...
	clock.CallbackAfter(t0.Add(slotDuration).Add(time.Second), func() {
		// Since the expected public keys are never the same as what the duty returns,
		// no epoch gets resolved, ever.

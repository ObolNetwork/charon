...
func randomMsg(t *testing.T) *pbv1.QBFTMsg {
	...
	vSet, err := core.UnsignedDataSetToProto(testutil.RandomUnsignedDataSet(t))
	require.NoError(t, err)
	pvSet, err := core.UnsignedDataSetToProto(testutil.RandomUnsignedDataSet(t))
	require.NoError(t, err)

	v, err := anypb.New(vSet)
	require.NoError(t, err)
	pv, err := anypb.New(pvSet)
	require.NoError(t, err)

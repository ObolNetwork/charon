...
func TestGenerateTSS(t *testing.T) {
	...
	tss, secrets, err := crypto.GenerateTSS(threshold, shares, rand.Reader)
...
func TestAggregateSignatures(t *testing.T) {
	...
	for i, secret := range secrets {
		psig, err := crypto.PartialSign(secret, msg)
		require.NoError(t, err)

		partialSigs[i] = psig
	}

	sig, _, err := crypto.AggregateSignatures(tss, partialSigs, msg)
	require.NoError(t, err)

	result, err := crypto.Verify(tss.PubKey, msg, sig)
	require.NoError(t, err)

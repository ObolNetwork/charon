...
#
# This script merges cluster-lock.json and key files for one node of a cluster.
# To merge full cluster, consider running cluster_merge.sh.
#
...
function usage() {
    echo "Usage: $0 dst_node_folder src_node_folder"
    echo ""
    echo "  dst_node_folder: Path to the destination node's folder"
    echo "                   (e.g., path/to/cluster1/node0)"
    echo "  src_node_folder: Path to the source node's folder"
    echo "                   (e.g., path/to/cluster2/node0)"
    exit 1
}

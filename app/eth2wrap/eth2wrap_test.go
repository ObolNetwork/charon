...
func TestErrors(t *testing.T) {
	...
	t.Run("network dial error", func(t *testing.T) {
		cl, err := eth2wrap.NewMultiHTTP(time.Hour, [4]byte{}, []string{}, "localhost:22222")
		require.NoError(t, err)
	...
	t.Run("http timeout", func(t *testing.T) {
		cl, err := eth2wrap.NewMultiHTTP(time.Millisecond, [4]byte{}, []string{}, srv.URL)
		require.NoError(t, err)
...
func TestCtxCancel(t *testing.T) {
	for range 10 {
		...
		bmock, err := beaconmock.New()
		require.NoError(t, err)
		eth2Cl, err := eth2wrap.NewMultiHTTP(time.Second, [4]byte{}, []string{}, bmock.Address())
		require.NoError(t, err)
...
func TestOneError(t *testing.T) {
	...
	eth2Cl, err := eth2wrap.NewMultiHTTP(time.Second, [4]byte{}, []string{}, addresses...)
	require.NoError(t, err)
...
func TestOneTimeout(t *testing.T) {
	...
	eth2Cl, err := eth2wrap.NewMultiHTTP(time.Minute, [4]byte{}, []string{}, addresses...)
	require.NoError(t, err)
...
func TestOnlyTimeout(t *testing.T) {
	...
	eth2Cl, err := eth2wrap.NewMultiHTTP(time.Minute, [4]byte{}, []string{}, srv.URL)
	require.NoError(t, err)
...
func TestLazy(t *testing.T) {
	...
	eth2Cl, err := eth2wrap.NewMultiHTTP(time.Second, [4]byte{}, []string{}, srv1.URL, srv2.URL)
	require.NoError(t, err)
...
func TestLazyDomain(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			require.NoError(t, err)
			eth2Cl, err := eth2wrap.NewMultiHTTP(time.Second, [4]byte(forkVersionHex), []string{}, srv.URL)
			require.NoError(t, err)

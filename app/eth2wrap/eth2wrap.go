...
func NewMultiHTTP(timeout time.Duration, forkVersion [4]byte, fallbackAddresses []string, addresses ...string) (Client, error) {
	return InstrumentWithFallback(
		NewFallbackClient(timeout, forkVersion, fallbackAddresses),
		newClients(timeout, forkVersion, addresses)...,
	)
}

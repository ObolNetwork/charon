...
Note: PRs can only be merged by obol-bulldozer bot. It is authors' responsibility to add label `merge when ready` after getting approval.
...
Different **dev tools** are used throughout the code base and are defined and installed from [go.mod](../go.mod). To install the dev tools run: `go install tool`

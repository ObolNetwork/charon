...
The above `cluster-definition.json` is generated by the [dv-launchpad](https://launchpad.obol.org/) provided as input to the `charon dkg` command which generates keys and the `cluster-lock.json` file.

...
| `app_beacon_node_sse_chain_reorg_depth` | Gauge | Chain reorg depth, supplied by beacon node`s SSE endpoint | `url` |
| `app_beacon_node_sse_head_delay` | Histogram | Delay in ms between the beacon node head and the SSE head | `url` |
| `app_beacon_node_sse_head_slot` | Gauge | Current beacon node head slot, supplied by beacon node`s SSE endpoint | `url` |
